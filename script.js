...
function displayQuestion() {
    ...
    currentChoices = [questions[currentQuestionIndex].correctAnswer].concat(
        questions[currentQuestionIndex].incorrectAnswers,
    );
    correctAnswer = questions[currentQuestionIndex].correctAnswer;
    currentChoices.sort(() => Math.random() - 0.5); // Shuffle choices

    for (let i = 0; i < 4; i++) {
        const btn = document.getElementById(`choice${i + 1}`);
        btn.innerHTML = currentChoices[i];
        btn.value = currentChoices[i]; // Add value for checking answers
    }
}
...
function nextQuestion() {
    ...
    if (currentQuestionIndex < questions.length) {
        ...
        // Optionally, disable buttons or show final score
    }
}
